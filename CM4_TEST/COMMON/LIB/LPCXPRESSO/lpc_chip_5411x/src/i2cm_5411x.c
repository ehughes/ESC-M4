/*
 * @brief LPC5411x I2C master driver
 *
 * @note
...
{
	uint32_t scl, div;
	uint32_t clk = Chip_Clock_GetFLEXCOMMClockRate(Chip_FLEXCOMM_GetIndex(pI2C));

	/* Check if CLK is a sane value */
	LPC_ASSERT(clk, __FILE__, __LINE__);

	for (scl = 9; scl >= 2; scl --) {
		div = clk / (busSpeed * (scl << 1));
		if (div * (busSpeed * (scl << 1)) == clk) break;
	}

	Chip_I2C_SetClockDiv(pI2C, div);
	Chip_I2CM_SetDutyCycle(pI2C, scl, scl);
}
