...
PINCFGTABLEFLASH
				...
				DCB		1							; ifSel:			Interface selection for host (0,=AUTODETECT, 1=I2C0, 2=I2C1, 3=I2C2, 4=SPI0, 5=SPI1)
				DCB		((0 << 5) + 30)				; hostIrqPortPin:	Host IRQ port (bits 7:5) and pins (bits 4:0)
				DCB		((1 << 5) + 4)				; hostMisoPortPin:	SPI MISO port (bits 7:5) and pins (bits 4:0)
				...
				DCB		0 ^ 1 ^ ((0 << 5) + 30) ^ ((1 << 5) + 4) ^ ((0 << 5) + 12) ^ ((0 << 5) + 14) ^ ((1 << 5) + 3)	; checksum:			
                EXPORT  CRC32_LEN
                EXPORT  CRC32_VAL
CRC32_LEN       DCD     0x00000950							; Length for CRC32 check starting at offset 0, in 32-bit words
CRC32_VAL       DCD     0xfd6a867c 							; CRC32 value
