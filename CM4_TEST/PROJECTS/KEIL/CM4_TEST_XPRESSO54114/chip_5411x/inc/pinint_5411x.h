/*
 * @brief LPC5411X Pin Interrupt and Pattern Match Registers and driver
 *
 * @note
...
#ifndef __PININT_5411X_H_
#define __PININT_5411X_H_

#ifdef __cplusplus
extern "C" {
#endif

/** @defgroup PININT_5411X CHIP: LPC5411X Pin Interrupt and Pattern Match driver
 * @ingroup CHIP_5411X_DRIVERS
 * @{
 */

/**
 * @brief LPC5411X Pin Interrupt and Pattern Match register block structure
 */
...
/**
 * LPC5411X Pin Interrupt and Pattern match engine register
 ...
 * @return	Nothing
 */
__STATIC_INLINE void Chip_PININT_SetPatternMatchSrc(LPC_PIN_INT_T *pPININT,
									Chip_PININT_SELECT_T channelNum,
									Chip_PININT_BITSLICE_T sliceNum)
{
	uint32_t idx = PININT_SRC_BITSOURCE_START + (sliceNum * 3);

	/* Source source for pattern matching */
	pPININT->PMSRC = (pPININT->PMSRC & ~(PININT_SRC_BITSOURCE_MASK << idx)) |
			(channelNum << idx);
}
 ...
 * @return	Nothing
 */
__STATIC_INLINE void Chip_PININT_SetPatternMatchConfig(LPC_PIN_INT_T *pPININT,
		Chip_PININT_BITSLICE_T sliceNum,
		Chip_PININT_BITSLICE_CFG_T slice_cfg, bool end_point)
{
	uint32_t pmcfg, idx = PININT_SRC_BITCFG_START + (sliceNum * 3);

	/* Configure bit slice configuration */
	pmcfg = pPININT->PMCFG;
	pmcfg = (pmcfg & ~(PININT_SRC_BITCFG_MASK << idx)) | (slice_cfg << idx);
