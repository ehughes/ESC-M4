 ...
 *                                 |_|
 */
void eGFX_PutPixel(const eGFX_ImagePlane *Image,
                   int16_t x,
                   int16_t y,
                   eGFX_PixelState PS);
...
void eGFX_ProcessAnimators(void);
...
void eGFX_ClearAnimators(void);

//Gets the number of animator objects in the list
uint32_t eGFX_GetAnimatorCount(void);

//Returns True if the animator is complete
uint32_t eGFX_AnimatorIsComplete(void * A);

//returns TRUE if all of the animators are complete
uint32_t eGFX_AnimatorsAreComplete(void);

//Pauses all the animators (sets the state to inactive)
void eGFX_PauseAnimators(void);

//Starts all the Animators in the list.  (Sets the State to Active)
void eGFX_StartAnimators(void);

//Resets and Animator to step 0 and starts it again.
void eGFX_ResetAnimator(void * A);

//Resets and restarts all animators
void eGFX_ResetAnimators(void);
...
void eGFX_ClearObjects(void);

//Gets the number of objects in the object list
uint32_t eGFX_GetAnimatorCount(void);

//Disables rendering/processing of the object
void eGFX_DisableObject(void * Object);

//enable rendering/processing of the object
